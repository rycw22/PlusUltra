...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        help='dataset path',
        default=f"/media/ameyer/Data4/EchoCP2/EchoCP_dataset")
    parser.add_argument(
        '--save-dir',
        type=str,
...
def main() -> None:
    ...
    coco = CocoAnnotationObject(
        ...
        dataset_url="https://www.kaggle.com/datasets/xiaoweixumedicalai/echocp"
    )
    coco.set_categories([
        {"supercategory": "chamber", "id": 1, "name": "chamber"},
    ])
    img_filenames = list(scandir(args.path, suffix=("nii.gz",), recursive=True))

    mapping = defaultdict(lambda: defaultdict(str))
    for filename in img_filenames:
        idx = "_".join(filename.split("/")[-1].split(".")[0].split("_")[:-1])
        modality = "img"
